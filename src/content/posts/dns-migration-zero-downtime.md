...
192.0.2.64 lb-qa.domain.local lb-qa
192.0.2.65 vrrp-qa.domain.local vrrp-qa
```
...
```
BEFORE:
Name: lb-qa.domain.local → IP: 192.0.2.100 (old server)

AFTER:
Name: lb-qa.domain.local → IP: 192.0.2.64 (new server)
```
...
![DNS Migration Strategy for Zero-Downtime System Replacement](/images/diagrams/new/dns-migration.svg)
