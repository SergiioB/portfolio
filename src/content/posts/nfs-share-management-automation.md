...
/srv/nfs_share_project \
  192.0.2.128/27(rw,no_root_squash,sync,no_subtree_check) \
  192.0.2.160/27(rw,no_root_squash,sync,no_subtree_check) \
  203.0.113.128/27(rw,no_root_squash,sync,no_subtree_check)
```
...
mount -t nfs4 -o rw,bg,hard,nointr,rsize=32768,wsize=32768,tcp,timeo=600,_netdev \
  198.51.100.102:/srv/oracle_pbb_export_share/oracle_pbb_export_share \
...
/srv/oracle_share \
  ...
  198.51.100.102(rw,no_root_squash)
```
...
/srv/oracle_data \
  198.51.100.0/24(rw,no_root_squash,sync,no_subtree_check) \
  192.0.2.0/24(rw,no_root_squash,sync,no_subtree_check)
```
...
```bash
mount -t nfs4 -o rw,bg,hard,nointr,rsize=32768,wsize=32768,tcp,timeo=600,_netdev \
  198.51.100.100:/srv/oracle_data \
  /oracle/oradata
```
...
```bash
198.51.100.100:/srv/oracle_data /oracle/oradata nfs4 rw,bg,hard,nointr,rsize=32768,wsize=32768,tcp,timeo=600,_netdev 0 0
```
...
![Automating NFS Share Management at Scale](/images/diagrams/new/nfs-share-management-automation.svg)
...
```bash
exportfs -ra
mount -t nfs4 -o rw,bg,hard,nointr,rsize=32768,wsize=32768 198.51.100.102:/export /mnt
