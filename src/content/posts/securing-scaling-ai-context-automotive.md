...
usedIn: "Used in the Node.js Firebase backend of IntelliAuto—a production AI-powered automotive maintenance app live on Google Play."
...
This post covers the backend security architecture for **IntelliAuto's AutoMind AI assistant**. For a complete overview including the Android app architecture, mobile security, and monetization strategy, see:

**→ [IntelliAuto: AI-Powered Automotive Assistant with Secure Monetization](/posts/intelliauto-ai-mechanic-assistant)**
...
- **Free Tier:** Routed to a standard chat model.
- **Pro Tier:** Routed to an advanced reasoning model.
...
- **Conversational Memory:** Only the most recent messages are retained, with each message hard-capped at a fixed character limit.
- **Data Injection:** Vehicle maintenance records are sorted by date, and only the most recent records are injected into the context.
...
1.  **Cooldowns:** A strict cooldown between queries. The backend checks a timestamp in the database. If the delta is too short, it throws an immediate error without calling the LLM.
...
User input is never trusted. Before a query even reaches the context builder, it passes through a sanitization function.

This function:

1.  Strips out zero-width and control characters.
2.  Enforces a strict character limit on the raw question.
3.  Runs the text against a set of regex definitions looking for common injection patterns (instruction override, code execution requests, role manipulation).

If a pattern matches, the request is immediately rejected with a detection flag, keeping the system prompt secure.
