...
const CATEGORY_NOTES = {
  infrastructure: {
    ...
    sequence: [
      ...
      "Apply change in controlled stage",
      "Run post-change validation",
      "Document handoff and rollback point",
    ],
  },
  automation: {
    ...
    sequence: [
      ...
      "Execute automation with guardrails",
      "Collect outputs and drift signals",
      "Iterate runbook from telemetry",
    ],
  },
  ai: {
    ...
    sequence: [
      ...
      "Run model task with constraints",
      "Review output quality and fallback behavior",
    ],
  },
  cloud: {
    focus: "cost-aware operations, resiliency, and secure service boundaries",
    sequence: [
      "Define service boundary and SLO",
      "Deploy with policy checks",
      "Observe latency/errors/cost",
      "Tune capacity and controls",
    ],
  },
  "local-ai": {
    focus: "memory budgeting, latency behavior, and stable edge inference",
    sequence: [
      "Measure baseline runtime footprint",
      "Tune quantization/context/runtime flags",
      "Benchmark latency and memory impact",
      "Select production-safe profile",
    ],
  },
  kotlin: {
    focus: "runtime performance, lifecycle correctness, and maintainable app architecture",
    sequence: [
      "Isolate UX or performance issue",
      "Refactor with clear layer boundaries",
      "Validate on target devices",
      "Track regressions through repeatable tests",
    ],
  },
  snippets: {
    focus: "fast remediation patterns turned into reusable operational playbooks",
    sequence: [
      "Use targeted snippet for incident recovery",
      "Validate outcome and side effects",
      "Standardize command pattern",
      "Promote into documented runbook",
    ],
  },
  career: {
    ...
    sequence: [
      ...
      "Execute with stakeholder alignment",
      "Measure operational result",
      "Capture lessons and next-level ownership",
    ],
  },
};
