...
const categories = [
  { title: "File & Directory Mgmt", items: [
  ...
  { title: "Text Processing & Search", items: [
  ...
  { title: "Archiving, Compress & Sync", items: [
  ...
  { title: "Firewalld & Iptables", items: [
  ...
  { title: "System & Process Monitor", items: [
  ...
  { title: "Storage & Filesystems", items: [
  ...
  { title: "LVM & Disk Expansion", items: [
  ...
  { title: "User, Permission & Audit", items: [
  ...
  { title: "Git Version Control", items: [
  ...
  { title: "Python Context Mgmt", items: [
  ...
  { title: "Ansible Core & Execution", items: [
  ...
  { title: "Ansible Vault & Secrets", items: [
  ...
  { title: "DBs, SELinux & Certs", items: [
    { cmd: "vim /data/postgres/pg_hba.conf", desc: "Edit Postgres client auth policy" },
    { cmd: "sudo -u postgres psql -c 'SELECT pg_reload_conf();'", desc: "Reload Postgres gracefully" },
    { cmd: "sestatus", desc: "Verify SELinux enablement state" },
    { cmd: "audit2allow -a", desc: "Generate allow rules from audit logs" },
    { cmd: "semanage fcontext -a -t postgresql_db_t '/data(/.*)?'", desc: "Define SELinux contexts" },
    { cmd: "restorecon -Rv /data", desc: "Apply SELinux fcontexts recursively" },
    { cmd: "openssl req -new -newkey rsa:2048 -nodes -keyout k.key -out c.csr", desc: "Generate CSR immediately" },
    { cmd: "openssl x509 -in tls.crt -text -noout", desc: "Inspect x509 certificate data" },
    { cmd: "openssl s_client -connect host:443 -showcerts", desc: "Examine remote SSL certificate" },
    { cmd: "certbot certificates", desc: "List locally managed ACME certs" },
    { cmd: "/opt/setup.sh -silent -responsefile props", desc: "Run enterprise installer silently" }
  ]}
];

const colors = ["#8aadf4", "#8bd5ca", "#a6da95", "#f5a97f", "#eed49f", "#c6a0f6", "#ed8796", "#f5bde6"];
let colorIdx = 0;
const columns = [
  { x: 100, y: 250 },
  { x: 1135, y: 250 },
...
for (const cat of categories) {
  ...
  svgNodes.push(`<rect x="${currentX}" y="${currentY}" width="930" height="${blockHeight}" rx="15" fill="#363a4f" stroke="${color}" stroke-width="3" />`);
  svgNodes.push(`<text x="${currentX + 30}" y="${currentY + 65}" font-family="sans-serif" font-size="36" font-weight="bold" fill="${color}">${escapeXml(cat.title)}</text>`);
  svgNodes.push(`<line x1="${currentX}" y1="${currentY + 90}" x2="${currentX + 930}" y2="${currentY + 90}" stroke="#494d64" stroke-width="2" />`);
  
  for (const node of itemNodes) {
    if (node.class === "cmd") {
      svgNodes.push(`<text x="${currentX + node.x}" y="${currentY + node.yDiff}" font-family="monospace, Courier New" font-size="24" font-weight="bold" fill="#cad3f5">${node.text}</text>`);
    } else {
      svgNodes.push(`<text x="${currentX + node.x}" y="${currentY + node.yDiff}" font-family="sans-serif" font-size="22" fill="#a5adcb">${node.text}</text>`);
    }
  }
...
const outputPath = 'C:/Users/Sergiio/Syncthing/portfolio/public/images/diagrams/new/full-linux-engineer-command-list.svg';
const pngPath = 'C:/Users/Sergiio/Syncthing/portfolio/public/images/diagrams/new/full-linux-engineer-command-list.png';
