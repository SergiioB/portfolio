...
const CATEGORY_BASE = {
  infrastructure: {
    ...
    checks: [
      ...
      "`journalctl -p err -b` has no new regressions",
      "critical endpoint checks pass from at least two network zones",
    ],
  },
  automation: {
    ...
    checks: [
      "`ansible-playbook --check --diff` highlights only intended drift",
      "idempotency run returns `changed=0` for stable hosts",
      "rollback playbook is executable without ad-hoc edits",
    ],
  },
  ai: {
    ...
    checks: [
      ...
      "confidence/fallback policy routes low-quality outputs safely",
      "observability captures latency + quality per request class",
    ],
  },
  cloud: {
    ...
    checks: [
      "SLO dashboard remains within target after rollout",
      "autoscaling and quotas stay inside guardrails",
      "security policy checks pass in CI and runtime",
    ],
  },
  "local-ai": {
    ...
    checks: [
      "runtime memory stays under planned ceiling during peak context",
      "decode latency remains stable across repeated runs",
      "fallback model/profile activates cleanly when pressure increases",
    ],
  },
  kotlin: {
    ...
    checks: [
      "critical user flows complete without navigation regressions",
      "performance traces show smoother frame delivery",
      "shortcut/deeplink paths remain deterministic across app states",
    ],
  },
  snippets: {
    ...
    checks: [
      "commands are safe against common edge cases",
      "runbook version includes pre-check and post-check gates",
      "handoff notes specify ownership and escalation path",
    ],
  },
  career: {
    ...
    checks: [
      ...
      "execution tradeoffs are documented",
      "outcome metrics are tied to business or operational value",
    ],
  },
};
...
const KEYWORD_PROFILES = [
  {
    ...
    risks: [
      [
        "Inventory scope error",
        "Wrong hosts receive a valid but unintended change",
        "Use explicit host limits and pre-flight host list confirmation",
      ],
      [
        "Role variable drift",
        "Different environments behave inconsistently",
        "Pin defaults and validate required vars in CI",
      ],
      [
        "Undocumented manual step",
        "Automation appears successful but remains incomplete",
        "Move manual steps into pre/post tasks with assertions",
      ],
    ],
  },
  {
    ...
    risks: [
      [
        "Incorrect device target",
        "Data loss risk increases immediately",
        "Require device mapping verification and maintenance window gate",
      ],
      [
        "Insufficient free extents",
        "Resize fails mid-operation",
        "Pre-calculate growth/shrink plan before execution",
      ],
      [
        "Rollback ambiguity",
        "Recovery time extends during incident",
        "Create snapshot/backup and rollback notes ahead of change",
      ],
    ],
  },
  {
    ...
    risks: [
      [
        ...
        "Memory pressure causes latency spikes or OOM",
        "Tune ctx + cache quantization from measured baseline",
      ],
      [
        "Silent quality drift",
        "Outputs degrade while latency appears fine",
        "Track quality samples alongside perf metrics",
      ],
      [
        "Single-profile dependency",
        "No graceful behavior under load",
        "Define fallback profile and automatic failover rule",
      ],
    ],
  },
  {
    ...
    risks: [
      [
        ...
        "UX differs across OEM implementations",
        "Test across at least one mid and one high-tier device",
      ],
      [
        "Navigation edge case",
        "Deep links break when app state is partial",
        "Normalize entry routing through a single handler",
      ],
      [
        "Performance regression",
        "Small UI changes impact frame pacing",
        "Track frame timing in CI/perf checks",
      ],
    ],
  },
  {
    ...
    risks: [
      [
        "Auth or trust mismatch",
        "Service looks up but rejects real traffic",
        "Validate identity chain and clock/DNS assumptions",
      ],
      [
        "Policy-control conflict",
        "SELinux/firewall blocks valid paths",
        "Capture allow-list requirements before rollout",
      ],
      [
        "Partial restart strategy",
        "Config is applied but not activated safely",
        "Use staged restart with health gates",
      ],
    ],
  },
  {
    ...
    risks: [
      [
        "Pipeline secret drift",
        "Deploy path fails unexpectedly",
        "Pin secret names and validate before deploy step",
      ],
      [
        "Notification-only success",
        "Chat alert says success while endpoint is broken",
        "Gate notifications on real health checks",
      ],
      [
        "Environment mismatch",
        "Prod/staging behavior diverges",
        "Use explicit environment matrix in pipeline config",
      ],
    ],
  },
];
...
for (const file of files) {
  ...
  const risks = profile
    ...
    : [
        [
          "Scope ambiguity",
          "Teams execute different interpretations",
          "Write explicit pre-check and success criteria",
        ],
        [
          "Weak rollback plan",
          "Incident recovery slows down",
          "Define rollback trigger + owner before rollout",
        ],
        [
          "Insufficient telemetry",
          "Failures surface too late",
          "Require post-change monitoring checkpoints",
        ],
      ];
